...
static volatile rt_uint8_t cache_buf1[SDIO_BUFF_SIZE], cache_buf2[SDIO_BUFF_SIZE];
...
static void rthw_sdio_wait_completed(struct rthw_sdio *sdio)
{
    ...
    else
    {
        ...
        {
            LOG_W("sdio:%p send command[%u] error = %d  status = %08X", sdio, cmd->cmd_code, cmd->err, status);
        }
        else
        {
            LOG_E("sdio:%p send command[%u] error = %d  status = %08X", sdio, cmd->cmd_code, cmd->err, status);
        }
    }
}
...
int rt_hw_sdio_init(void)
{
    ...
    host1 = sdio_host_create(&sdio_des1, (rt_uint8_t *)cache_buf1);
    if (host1 == RT_NULL)
    {
    ...
    host2 = sdio_host_create(&sdio_des2, (rt_uint8_t *)cache_buf2);
    if (host2 == RT_NULL)
    {
