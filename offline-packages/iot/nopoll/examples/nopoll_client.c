...
static int nopoll_client(int argc, char **argv)
{
	...
	if (argc < 3)
	{
		printf("Using: nopoll_client <host name> <port>\n");
		return -1;
	}
	...
	/* create connection */
	conn = nopoll_conn_new(ctx, argv[1], argv[2], argv[1], NULL, NULL, NULL);
...
MSH_CMD_EXPORT(nopoll_client, websocket client test);
